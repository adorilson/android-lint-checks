...
\section{Introdução}
    ...
    \frame
    {
        \frametitle{Trabalhos relacionados}
        \begin{itemize}
            \item Em [Mojica, 2014] o trabalho de [Mojica, 2012] é expandido para
            mais de 200 mil aplicações em 30 categorias do Google Play.
            \begin{itemize}
                \item Alguns resultados são significativamente diferentes, como
                \begin{itemize}
                    \item 18,75\% das classes herdam de classe da API do Android,
                    contra 23\% anteriormente
                    \item 35,78\% das classes herdam de classe específica do domínio,
                    contra 27\% anteriormente
                    \item 84,23\% das classes aparecem em duas ou mais aplicações,
                    considerando todas as categorias, contra 61\% anteriormente e
                    considerando apenas aplicações da mesma categoria
                    \item Também foram percebidas diferenças no ``Top 10'' classes
    ...
    \frame
    {
        \frametitle{Trabalhos relacionados}
        \begin{itemize}
            \item Em [Berger, 2014] é realizado um estudo sobre mecanismos de
            variabilidades em cinco ecosistemas: Android, o kernel Linux, Debian,
            Eclipse e o sistema operacional eCos. Suas contribuições foram:
            \begin{itemize}
                \item Um framework conceitual definindo características chaves dos
                mecanismos de variabilidades e seus contextos organizacionais através
                dos ecosistemas
                \item Uma instanciação do framework com dados empiricos para cada
                ecosistema;
                \item um conjunto de diferenças centrais entre os ecosistemas, e
                hipoteses;
                \item ferramentas para analise estática e conjuntos de dados extraídos
                para reprodução e trabalhos futuros
            \end{itemize}
        \end{itemize}
    }
    \frame
    {
        \frametitle{Trabalhos relacionados}
        \begin{itemize}
            \item Algumas considerações gerais:
            \begin{itemize}
                \item O artigo é denso, complexo. Foi feito um trabalho
                pesado pelos autores de comparação dos cinco ecosistemas avaliados.
                \item Apenas pelo artigo, não ficou claro como a analise
                estática foi útil para a definição do framework e dos
                mecanimos de variabilidade
                \item Para entender isso e outros aspectos, é necesssário
                ...
                Play não tem nenhuma garantia de qualidade das aplicações.
...
\section{Android Lint}
   ...
   \frame
   {
        ...
        Além de herdar de FragmentActivity, é necessário de fato usar fragmentos.
        Verificamos isso se no método onCreate() da activity, de forma direta ou
        indireta, é feita uma chamada para o método beginTransaction() da 
   ...
   \frame
   {
        ...
        Além da activity herdar de ActionBarActivity, também é necessário usar
        um estilo específico: @style/Theme.AppCompat.Light. O check apresentado
        a seguir faz esse verificação.
        
        Serão apresentados os seguintes elementos:
        \begin{itemize}
            \item ThemeDetector: classe detector
            \begin{itemize}
                \item Este detector foi feito através da extensão de um outro
                preexistente e não apenas a partir de classes da API, como os demais.
            \end{itemize}
    ...
    \frame
    {
        ...
        \begin{itemize}
            ...
            \begin{itemize}
                ...
                ser aprimorado para suportar heranças indiretas
            \end{itemize}
            ...
            tivemos uma boa variação de elementos utilizados da API, devido aos distintos
            objetivos desses checks. São analisados: arquivos XML, código-fonte Java e bytecode.
        \end{itemize}
    }
